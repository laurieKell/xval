...
u=diags:::readVP2BoxIndices(file.path(dirVpa,"new/bfte2017.d1"))
names(u)=c("ESPMarTrap","MarPorTrap","JLL EastMed","JP LL NEA1","JP LL NEA2",
           "SP BB1",    "SP BB2",    "Aerial",     "larval survey")
...
waa=read.table(file.path(dirVpa,"new/bfte2017.d1"),skip=428,nrows=483-428)
waa=waa[waa[,1]%in%c(6,7,9),]
...
paa=read.table(file.path(dirVpa,"new/bfte2017.d1"),skip=264,nrows=422-264)
...
baa=ddply(subset(paa, (name%in%nms)),.(name,decade), transform, p=wt*p/sum(wt*p))
```
...
paa=read.table(file.path(dirVpa,"new/bfte2017.d1"),skip=262,nrows=415-262)
names(paa)[1:2]=c("name","year")
paa=melt(paa,id=c("name","year"))
...
```{r cca}
caa=read.table(file.path(dirVpa,"new/bfte2017.d1"),skip=24,nrows=71-24)
names(caa)[1]="year"
