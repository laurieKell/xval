...
setMethod("refpts<-", signature(object="FLBRP", value="FLPar"),
  function(object, value) object@refpts=value)

setGeneric('refpts', function(object, ...)
		standardGeneric('refpts'))

setMethod("refpts", signature(object="FLBRP"),
  function(object) object@refpts)

load(file.path(dirDat,"oms.RData"))

eqls=as(mlply(seq(length(oms)), function(x,om=oms,sr=srr) brp(FLBRP(om[[x]],sr=sr[[x]],nyears=5))),"FLBRPs")
...
#load("/home/laurence/Desktop/tmp/dat.RData")
...
save(par,file="/home/laurence/Desktop/tmp/par.RData")
```

```{r}
load("/home/laurence/Desktop/tmp/par.RData")
...
save(star,file=file.path(dirDat,"star.RData"))
...
```{r OM-srrRegime}
shifts=ddply(star,.(X1), transmute, shift=c(data[-1],NA)-data)[,2]

shifts=shifts[!is.na(shifts)]

hist(shifts)

save(shifts,file=file.path(dirDat,"shifts.RData"))
```

**Figure `r iFig=iFig+1; iFig`.** Distributions of regimes shifts in recruitments
...
source('/home/laurence/Desktop/flr/diags/R/FLdiags/diags-MFCL.R')
...
```{r}
save(dgs,file=file.path(dirDat,"dgs-bd.RData"))
```
